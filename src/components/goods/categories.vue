...
<div> 
    ...
    <el-card class="box-card"> 
        ...
        <!-- .FIXME: 表格区域 -->
           ...
           <!-- expand-type 展开行 -->
           <!-- is-fold 默认 展开数据 -->
        <tree-table :data='cateList' :columns='columns' :selection-type='false' stripe border show-index  :expand-type='false' show-header :is-fold='true'
        class="tree_table">
             ...
             <template slot="deleted" slot-scope="scope">
                 ...
                 <span class="el-icon-success" v-if="scope.row.cat_deleted == false"></span>
          ...
          <template slot="operate" slot-scope="scope">
              ...
              <!-- 两个按钮 -->
              <el-button type="warning" icon="el-icon-edit" size="mini" @click="editGoods(scope.row.cat_id)">编辑</el-button>
              <el-button type="danger" icon="el-icon-delete" size="mini" @click='delGoods(scope.row.cat_id)'>删除</el-button>
      ...
      background>
       ...
       <!-- .FIXME: 添加商品分类的对话框 -->
       <el-dialog  title="添加分类"  :visible.sync="addGoodsVisible"  width="50%" @close='addGoodsClose'>

           <!-- TODO 表单区域 -->
           <!-- 先给 Form 属性 绑定一个 addGoodsData 对象,再给 input表单元素 双向绑定  addGoodsData对象里面的属性 -->
           <!-- 需要添加 prop,不然验证规则没用 -->
             <el-form :model="addGoodsList" :rules="addGoodsRules" ref="addGoodsRef" abel-width="80px" >
             ...
             <span slot="footer" class="dialog-footer">
                <el-button @click="addGoodsVisible = false" type="warning">取 消</el-button>
      ...
      <!-- .FIXME: 编辑商品分类的对话框 -->
      <el-dialog title="编辑商品分类" :visible.sync="editGoodsVisible"  width="50%" @close='editGoodsClose'>


           <!-- TODO 表单区域 -->
           <!-- 先给 Form 属性 绑定一个 addGoodsData 对象,再给 input表单元素 双向绑定  addGoodsData对象里面的属性 -->
           <!-- 需要添加 prop,不然验证规则没用 -->
             <el-form :model="editGoodsList" :rules="editGoodsRules" ref="editGoodsRef" abel-width="80px" >
             ...
             <span slot="footer" class="dialog-footer">
                <el-button @click="editGoodsVisible = false" type="warning">取 消</el-button>
    ...
    methods:{
      ...
      async  getCateList(){
        ...
        async showAddCateData(){
            ...
            if(res.meta.status !=200)  return this.$message.error('获取二姐商品分类失败')
        ...
        handleChange(){
            ...
            if(this.selectValue == 0) {
                ...
                // 分类父id取选中id的数组最后一项
                this.addGoodsList.cat_pid = this.selectValue[this.selectValue.length-1];
            }
        },
        ...
        addGoodsSure(){
            ...
            this.$refs.addGoodsRef.validate( async boo=>{
                ...
                // 发送请求
              const {data:res} = await  this.$http.post('categories',this.addGoodsList)
        ...
        editGoodsClose(){
         ...
         async editGoods(id){
            ...
            if(res.meta.status != 200) return this.$message.error('获取当前分类数据失败')
        ...
        // TODO  编辑分类 将数据同步到数据库
          postGoods(){
               ...
               // 2.发送请求 将修改后的数据同步到数据库
                const {data:res} = await this.$http.put(`categories/${this.editGoodsList.cat_id}`,{
