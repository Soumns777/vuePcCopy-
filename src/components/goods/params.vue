  ...
  <div>
     ...
     <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-card class="box-card">
           ...
           <!-- 去除后面的 x 号 -->
           <el-alert title="只用于为三级分类设置相关参数" type="warning" show-icon :closable='false' center >
         ...
         <el-tabs v-model="activeName" @tab-click="handleTabClick" :stretch='false' type="border-card">
               
               <!-- .TODO:  动态参数面板 -->
            <el-tab-pane label="动态参数" name="many">
                <!-- 属性绑定  计算属性返回的布尔值,确认按钮的 禁用状态 (只有当选中的是三级分类时,下面的按钮才可以点) -->
                   <el-button type="primary" size="mini" :disabled='isDisabled' @click='addAttributeVisible = true'>添加参数</el-button>
                    ...
                    <!-- data 绑定的 激活状态为 many的面板的数据 -->
                     <el-table :data="manyData" style="width: 100%" stripe border> 
                         ...
                         <el-table-column type="expand" width="50" label="展开列">
                             ...
                             <template v-slot='scope'> 
                                ...
                                <!-- {{scope.row.inputVisible}} -->
                                   ...
                                   <!-- 循环展示的 tag标签 -->
                                   <!-- closable 可删除 -->
                                 <el-tag v-for="(item,i) in scope.row.attr_vals" :key="i" type="success" closable @close='delTag(i,scope.row)' >
                                      {{item}}  
                                 </el-tag>
                                ...
                                <!-- 添加新标签 -->
                                <el-button v-else class="button-new-tag" size="small" @click="showInput(scope.row)" type="success">+ New Tag</el-button>
                        ...
                        <!-- 索引列  -->
                          <el-table-column type="index" width="50" label="索引列"></el-table-column>
                        ...
                        <el-table-column label="操作" width="180">
                         ...
                         <!-- 用作用域插槽获取按钮 id -->
                         <template v-slot='scope'>
                            ID: {{scope.row.attr_id}}
                             <br><br>

                             <!-- 两个按钮 --> 
                             <el-button type="warning" class="el-icon-edit" size="mini" @click='editParams(scope.row) '>编辑</el-button>
                             <el-button type="danger" class="el-icon-delete" size="mini" @click='delParams(scope.row.attr_id)'>删除</el-button>
            ...
            <el-tab-pane label="静态属性" name="only">
                 <!-- 属性绑定  计算属性返回的布尔值,确认按钮的 禁用状态 (只有当选中的是三级分类时,下面的按钮才可以点) -->
            <el-button type="warning" size="mini" :disabled='isDisabled' @click='addAttributeVisible = true'>添加属性</el-button>
         ...
         <!-- 对话框的 close 事件 -->
         <el-dialog :title="'添加'+ text" :visible.sync="addAttributeVisible" width="50%" @close='addAttributeClose' >

              
           <!-- 表单区域 -->
           <el-form :model="addAttributeFormData" label-width="80px" ref='addAttributeRef' :rules="addAttributeRules">

               <el-form-item :label="text" prop="attr_name">

                   <el-input v-model="addAttributeFormData.attr_name">
             ...
             <span slot="footer" class="dialog-footer">
               <el-button @click="addAttributeVisible = false" type="warning">取 消</el-button>
         ...
         <!-- 可以属性绑定  :title='text' 渲染计算属性 -->
         <!-- 对话框的 close 事件 -->
         <el-dialog :title="'编辑'+ text" :visible.sync="editAttributeVisible" width="50%" @close='editAttributeClose' >

              
           <!-- 表单区域 -->
           <el-form :model="editAttributeFormData" label-width="80px" ref='editAttributeRef' :rules="addAttributeRules">

               <el-form-item :label="text" prop="attr_name">

                   <el-input v-model="editAttributeFormData.attr_name">
             ...
             <span slot="footer" class="dialog-footer">
               <el-button @click="editAttributeVisible = false" type="warning">取 消</el-button>
    ...
    methods:{
       ...
       async getCateList(){
        ...
        handleChange(){
            ...
            if(this.selectKeys.length == 0) return this.manyData = this.onlyData = [];
        ...
        async editParamsSure(){
             ...
             // 属性类型  this.activeName  激活状态的面板
              ...
              const {data:res} = await this.$http.put(`categories/${this.selectKeys[2]}/attributes/${this.editAttributeFormData.attr_id}`,{
        ...
        //   2.根据返回的字符串 判断是否删除改参数
          ...
          const {data:val} = await this.$http.delete(`categories/${this.selectKeys[2]}/attributes/${id}`)
