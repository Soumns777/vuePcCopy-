    ...
    <el-card class="box-card">
                ...
                <el-row v-for="item2 in item1.children" :key="item2.id">
                  ...
                  <el-col :span="16">
                    ...
                    <el-tag
                      ...
                      :key="item3.id"
                      @close="deleteRights(scoped.row, item3.id)"
        ...
        <el-table-column label="操作">
          ...
          <template v-slot="scoped">
            ...
            <el-button
              ...
              type="primary"
              icon="el-icon-edit"
              @click="editRoleMan(scoped.row.id)"
              >编辑</el-button
            >
            <el-button
              size="mini"
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="danger" @click="addRoleVisible = false"
            >取 消</el-button
          >
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          <!-- 点击取消退出对话框 -->
          <!-- 点击确认 进行  预校验 ==> 发送请求 ==> 重新渲染数据 ==> 清空表单 -->
          <el-button type="danger" @click="editRoleVisible = false"
            >Cancel</el-button
          >
      ...
      >
        ...
        <span slot="footer" class="editRightsVisible-footer">
          <el-button @click="editRightsVisible = false" type="warning"
            >取 消</el-button
          >
          <el-button type="primary" @click="postRightsByKeys">确 定</el-button>
        </span>
      </el-dialog>
    </el-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    addRoleMan() {
      ...
      this.$refs.addRoleRef.validate(async (res) => {
        ...
        if (val.meta.status != 201)
          return this.$message.error('新建角色请求失败')
    ...
    async removeRoleById(id) {
      ...
      // 根据返回的字符串确认 操作
      if (res != 'confirm')
        return this.$message.error('长点心吧弟弟,黑桃A都没了')
      ...
      if (val.meta.status != 200)
        return this.$message.error('连数据都删除不了的five')
    ...
    tiJiaoRole() {
      ...
      this.$refs.editRoleRef.validate(async (res) => {
        ...
        // 2.提交数据并重新渲染
        const { data: val } = await this.$http.put(
          `roles/${this.addRolesMan.roleId}`,
          {
    ...
    async deleteRights(role, rightId) {
      ...
      if (res != 'confirm')
        return this.$message.info('长点心吧徐爷的黑桃A都没了')
