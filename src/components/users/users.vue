     ...
     <!-- .TODO: 卡片区域 -->
      <el-card class="box-card">
          ...
          <el-row :gutter="15">
             ...
             <el-col :span="8"><el-input  v-model="queryInfo.query"  clearable @clear='getUserList'><el-button slot="append" icon="el-icon-search" @click='getUserList'></el-button></el-input></el-col>
             <el-col :span="5">  <el-button type="success" @click='addUserVisible = true' >添加用户</el-button></el-col>
          </el-row>     
          ...
          <el-table-column  label="操作"  >
              ...
              <template v-slot='scoped'>
                ...
                <!-- 点击修改的按钮 触发修改用户的对话框 -->
                  <el-button size="mini" type="primary" icon="el-icon-edit" @click='getUserData(scoped.row.id)'></el-button>
                  <el-button size="mini" type="warning" icon="el-icon-delete" @click='removeUserById(scoped.row.id)'></el-button>
                
                 <!-- 移入按钮会有提示信息 -->
                 <el-tooltip class="item" effect="dark" content="分配角色啊弟弟" placement="top" >
                     <el-button size="mini" type="danger" icon="el-icon-setting" @click='setRoles(scoped.row)'></el-button>
                  </el-tooltip>
                ...
                <!-- .TODO: 分页器区域 -->
                 ...
                 >
                   ...
                   <!-- 点击确认 进行  预校验 ==> 发送请求 ==> 重新渲染数据 ==> 清空表单 -->
                       <el-button  type="danger"  @click="addUserVisible = false">取 消</el-button>
                 ...
                 >
                   ...
                   <!-- 点击取消退出对话框 -->
                   <!-- 点击确认 进行  预校验 ==> 发送请求 ==> 重新渲染数据 ==> 清空表单 -->
                       <el-button  type="danger"  @click="editUserVisible = false">取 消</el-button>
                       <el-button type="primary"  @click="editUserForm">确 定</el-button>
                     </span>
                </el-dialog>

                <!-- .TODO: 分配角色对话框 -->
                <el-dialog title="分配角色" :visible.sync="setRolesVisible"
                 ...
                 <span slot="footer" class="dialog-footer">
                       ...
                       <el-button @click="setRolesVisible = false" type="warning">取 消</el-button>
...
export default {
    ...
    return  /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\.[a-zA-Z0-9_-])+/.test(value) ? callback() : callback(new Error('请输入合法的邮箱'))
    }

       // 手机号校验规则
         var  checkMobile = (rule,value,callback)=>{
    return /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/.test(value) ? callback() : callback(new Error('请输入合法的手机号'))
    }
    ...
    // 根据id发送请求获取数据
        ...
        async removeUserById(id){
         ...
         const res  = await this.$confirm('个是要把那个小次老子带走', '各单位注意', {
        ...
        // 5.根据返回的状态码判断 是否删除成功
        if(val.meta.status !=200) return this.$message.error('连数据都删除不了还删号码')
